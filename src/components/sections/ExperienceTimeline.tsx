...
function dotFor(entry: ExperienceEntry) {
  switch (entry.kind) {
    case 'work':
      return { icon: <WorkOutlined fontSize="small" />, color: 'primary' } as const
    case 'volunteer':
      return { icon: <VolunteerActivismOutlined fontSize="small" />, color: 'secondary' } as const
    case 'education':
      return { icon: <SchoolOutlined fontSize="small" />, color: 'info' } as const
    default: {
        ...
        {experience.map((entry, index) => {
          ...
          return (
            <TimelineItem key={`${entry.organization}-${entry.period}`}>
              ...
              <TimelineSeparator>
                <TimelineDot variant="outlined" color={dot.color} sx={{ my: 1 }}>
                  {dot.icon}
                </TimelineDot>
                    ...
                    <Stack spacing={1}>
                      <Stack spacing={0.25}>
                        <Typography variant="h6" sx={{ fontSize: '1.05rem' }}>
                          {entry.kind === 'education' ? entry.credential : entry.role}
                        </Typography>
